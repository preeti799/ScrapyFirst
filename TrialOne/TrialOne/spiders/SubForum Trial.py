...
class Maserati22(scrapy.Spider):
    name = "Maserati22"
    ...
    def parse(self, response):
        ...
        for i in sub_section_link:
            ...
            excluded_urls = ['/forums/4x4-shop-canada.241/', '/forums/a-s-motorsport.74/', '/forums/asia.33/',
                             ...
                             '/forums/europe.18/', '/forums/biturbo-maserati.16/', '/forums/vintage-maserati.8/',
                             '/forums/do-it-yourself-dyi.138/', '/forums/member-introductions.36/',
                             '/forums/garage-photos-videos.9/', '/forums/formula-dynamics.39/']
...
# IN THREAD PAGE:
    ...
    def parse_thread_page(self, response):
        ...
        thread_id = response.css("div.p-body-pageContent>script::text").re_first(r'\d+')
        ...
        for i in thread:
            sender_id = i.xpath('.//@id').get().split('_')[-1]
            receiver_id = i.xpath('.//a[@class="bbCodeBlock-sourceJump"]/@data-content-selector').extract_first(default='')
            receiver_name = i.xpath('.//a[@class="bbCodeBlock-sourceJump"]/text()').extract_first(default='')
            receiver_name_clean = receiver_name.replace(" said:", "")
            ...
            post_order_clean = post_order.strip().replace('\n', '').replace('#', '') if post_order else '1'

            yield {
                'Section_Name': section_name,
                'Thread_Title': thread_title,
                'Thread_ID': thread_id,
                'Thread_Views_Count': thread_views,
                'Thread_Reply_Count': thread_replies,
                'Thread_Participants_Count': thread_participants,
                'Sender_Name': sender_name,
                'Sender_ID': sender_id,
                'Receiver_ID': receiver_id,
                'Receiver_Name': receiver_name_clean,
                'Date': date,
                'Time': time,
                'Content': content,
                'Order': post_order_clean
            }
